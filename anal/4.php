...
/*end*/
//comments
//1. sw_get_value_by_id - лучше не использовать голый mysql, в d7 можно писать выражения, лучше делать через него
//2. подобные мелкие функции лучше вынести в отдельный файл и подключать его уже здесь
//3. использовать fetch() вместо GetNextElement() и GetFields()
//4. sw_get_option - название функции вводит в заблуждение, потому что внутри совсем другое происходит,
// непонятно какой option мы получаем, потмоу что по факту получаем значение свойства "рандомного" элемента"
// (при этом зачем-то юзается while, хотя мы получаем один элемент, тоже немного вводит в заблуждение),
//
//5.то же самое относится к sw_get_option_html и sw_get_option_img (возможно там просто перепутано и
    //фильтровать надо по id, а не по iblock_id